...
using Plateaumed.EHR.Security;
using Plateaumed.EHR.Timing;
    ...
    public class HostSettingsAppService : SettingsAppServiceBase, IHostSettingsAppService
    {
        ...
        private async Task<HostUserManagementSettingsEditDto> GetUserManagementAsync()
        {
            return new HostUserManagementSettingsEditDto
            {
                ...
                UserEmailSettings = new UserEmailSettingsEditDto()
                 {
                    EmailActivationExpirationHours = await SettingManager.GetSettingValueAsync<int>(AppSettings.Email.EmailActivationExpirationHours),
                    EnableEmailExpiration = await SettingManager.GetSettingValueAsync<bool>(AppSettings.Email.EnableEmailExpiration)
                 }
            };
        }
        ...
        private async Task UpdateUserManagementSettingsAsync(HostUserManagementSettingsEditDto settings)
        {
            ...
            await UpdateUserManagementEmailSettingsAsync(settings.UserEmailSettings);
        }
        ...
        private async Task UpdateUserManagementEmailSettingsAsync(UserEmailSettingsEditDto settings)
        {
            await SettingManager.ChangeSettingForApplicationAsync(
                AppSettings.Email.EmailActivationExpirationHours,
                settings.EmailActivationExpirationHours.ToString().ToLowerInvariant()
            );

            await SettingManager.ChangeSettingForApplicationAsync(
               AppSettings.Email.EnableEmailExpiration,
               settings.EnableEmailExpiration.ToString().ToLowerInvariant()
           );
        }
