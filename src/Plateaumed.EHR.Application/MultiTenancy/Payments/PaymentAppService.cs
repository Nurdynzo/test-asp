...
{
    public class PaymentAppService : EHRAppServiceBase, IPaymentAppService
    {
        ...
        public PaymentAppService(
            ...
            TenantManager tenantManager, IUserEmailer userEmailer)
        {
        ...
        public async Task<long> CreatePayment(CreatePaymentDto input)
        {
            ...
            using (UnitOfWorkManager.Current.SetTenantId(null))
            {
                ...
                tenant = await TenantManager.GetByIdAsync(AbpSession.GetTenantId());
            ...
            var Id = await _subscriptionPaymentRepository.InsertAndGetIdAsync(payment);

            var user = await GetCurrentUserAsync();
            await _userEmailer.SendEmailActivationLinkToFacilityUserAsync(user, targetEditionName, AppUrlService.CreateFacilityURL(tenant?.TenancyName));
        ...
        {
            if(targetEdition.Name == EditionManager.FreeTrialEditionName)
            {
                return 0;
            }
