...
{
    public class ApproveWalletFundingRequestsHandler : IApproveWalletFundingRequestsHandler
    {
        ...
        public ApproveWalletFundingRequestsHandler(
        ...
        public async Task Handle(WalletFundingRequestDto request, int tenantId)
        {
            ...
            await UpdateWalletBalance(wallet, request.AmountToBeFunded.ToMoney(), tenantId);
        ...
        private async Task UpdateWalletBalance(Wallet wallet, Money amountToBeFunded, int tenantId)
        {

            var pendingWalletFunds = await _walletHistoryRepository.GetAll()
                .Where(x => x.PatientId == wallet.PatientId
                && x.Source == TransactionSource.Indirect
                && x.Status == TransactionStatus.Pending).ToListAsync();

            var totalPendingFunds = pendingWalletFunds.Sum(x => x.Amount.Amount);

            if (totalPendingFunds != amountToBeFunded.Amount)
            {
                throw new UserFriendlyException("Amount to be funded is not valid");
            }

            wallet.Balance += amountToBeFunded;

            await _walletRepository.UpdateAsync(wallet);
            await UpdateWalletHistory(pendingWalletFunds, tenantId);
        }

        private async Task UpdateWalletHistory(List<WalletHistory> pendingWalletFunds, int tenantId)
        {


            foreach (var item in pendingWalletFunds)
            {
                ...
                await _walletHistoryRepository.UpdateAsync(item);
        ...
        private async Task InitializeInvoicePayment(Invoice invoice, Wallet wallet, IGrouping<long, WalletFundingItem> groupedItems)
        {
            foreach (var requestedInvoiceItem in groupedItems)
            {
                ...
                if (invoiceItem != null && invoiceItem.Status == InvoiceItemStatus.AwaitingApproval)
                {
