...
{
    public class SessionAppService : EHRAppServiceBase, ISessionAppService
    {
            ...
            return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                var output = new GetCurrentLoginInformationsOutput
                {
                    Application = new ApplicationInfoDto
                    {
                        ...
                        Currency = EHRConsts.Currency,
                        CurrencySign = EHRConsts.CurrencySign,
                        AllowTenantsToChangeEmailSettings = EHRConsts.AllowTenantsToChangeEmailSettings,
                ...
                if (AbpSession.UserId.HasValue)
                {
                    ...
                    if (user.StaffMemberFk != null)
                    {
                        output.User.StaffInfo.ServiceCentres = user.StaffMemberFk?.Jobs?.SelectMany(x => x.JobServiceCentres?.Select(y => y.ServiceCentre)).ToList();
                    }
                }
        ...
        protected virtual async Task<User> GetImpersonatorUserAsync()
        {
            using (CurrentUnitOfWork.SetTenantId(AbpSession.ImpersonatorTenantId))
            {
                ...
                var user = await UserManager.FindByIdExtendedAsync(new UserIdentifier(AbpSession.ImpersonatorTenantId, AbpSession.ImpersonatorUserId.Value));
                if (user == null)
                {
                    throw new Exception("User not found!");
                }
