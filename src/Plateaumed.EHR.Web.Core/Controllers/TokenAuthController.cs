...
using Plateaumed.EHR.Authentication.TwoFactor;
...
using Plateaumed.EHR.Web.Authentication.JwtBearer;
using Plateaumed.EHR.Web.Authentication.TwoFactor;
using Plateaumed.EHR.Web.Models.TokenAuth;
...
using Plateaumed.EHR.Net.Sms;
using Plateaumed.EHR.Notifications;
using Plateaumed.EHR.Security.Recaptcha;
using Plateaumed.EHR.Web.Authentication.External;
using Plateaumed.EHR.Web.Common;
...
namespace Plateaumed.EHR.Web.Controllers
{
    ...
    public class TokenAuthController : EHRControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                .ToString(EHRConsts.DateTimeOffsetFormat));
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            var loginResult = await GetLoginResultAsync(
                ...
                model.Password,
                model.TenancyName == null ? GetTenancyNameOrNull() : model.TenancyName
            );

            if (!loginResult.User.IsEmailConfirmed)
            {
                throw new UserFriendlyException("User email is not confirmed");
            }
        ...
        [HttpPost]
        public Task<bool> ValidateToken([FromBody] TokenVerificationModel model)
        {
            ...
            try
            {
                ...
                if (token != null)
                {
                    ...
                    jwtTokenHandler.ValidateToken(model.Token, validationParameters, out var validatedToken);
                    if (validatedToken is JwtSecurityToken jwtSecurityToken)
                    {
                        // we can do more comparison later.
            ...
            string refreshTokenKey = null)
        {
            ...
            string title = user.Title != null ? $"{user.Title}. " : string.Empty;
            claims.Add(new Claim(AppConsts.Name, $"{title}{user.Name}"));
            ...
            //TODO Temporary until we move to job switching
            var userExtended = await _userManager.FindByIdExtendedAsync(user.ToUserIdentifier());
            var primaryRole = userExtended.StaffMemberFk?.Jobs.FirstOrDefault(x => x.IsPrimary)?.JobRole?.Name;
            if (primaryRole != null)
            {
                claims.Add(new Claim(AppConsts.PrimaryRole, primaryRole));
            }
            // add facilityId
            long? facilityId = await _getCurrentUserFacilityIdQueryHandler.Handle(user.Id);
            if (facilityId > 0)
            {
                claims.Add(new Claim(AppConsts.FacilityId, facilityId.ToString()));
            }
