...
using Plateaumed.EHR.Authorization.Permissions;
...
namespace Plateaumed.EHR.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="EHRDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        protected readonly int DefaultTenantId = 1;

        protected AppTestBase()
        {
            SeedTestData();
            LoginAsDefaultTenantAdmin();
            var httpClient = Resolve<IHttpContextAccessor>();
            var context = httpClient.HttpContext = new DefaultHttpContext();
            context.User = new ClaimsPrincipal(new ClaimsIdentity(new List<Claim>
            { 
                new(AppConsts.FacilityId, "1"),
            }));
        }

        private void SeedTestData()
        {
            void NormalizeDbContext(EHRDbContext context)
            {
        ...
        protected void UsingDbContext(Action<EHRDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EHRDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<EHRDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<EHRDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EHRDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EHRDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EHRDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EHRDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<EHRDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<EHRDbContext, Task<TResult>> func)
        {
            TResult result;
        ...
        protected async Task LoginAsCustomTenantWithPermissions(params string[] permissions)
        {
            ...
            await SetUserPermissions(user, permissions);
        ...
        protected async Task SetUserPermissions(User user, params string[] permissions)
        {
            var validPermissions = Resolve<PermissionManager>().GetPermissionsFromNamesByValidating(permissions);
            await Resolve<UserManager>().SetGrantedPermissionsAsync(user, validPermissions);
        }
