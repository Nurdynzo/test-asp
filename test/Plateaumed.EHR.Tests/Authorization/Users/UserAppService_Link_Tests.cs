...
namespace Plateaumed.EHR.Tests.Authorization.Users
{
    ...
    public class UserAppService_Link_Tests : UserAppServiceTestBase
    {
        ...
        {
            var testTenant = new Tenant("Test", "test", TenantType.Individual)
            {
                ConnectionString = SimpleStringCipher.Instance.Encrypt("Server=localhost; Database=EHRTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;")
            };
